  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {tab === "buyers" && (
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div>
                <CardTitle className="text-sm font-semibold">Buyers</CardTitle>
                <p className="text-xs text-muted-foreground">Manage your international buyers — customers we export to.</p>
              </div>
              ...
              <table className="w-full text-xs">
                <thead className="border-b">
                  <tr>
                    <th className="text-left py-2 pr-4 font-semibold text-muted-foreground">Name</th>
                    <th className="text-left py-2 pr-4 font-semibold text-muted-foreground">Country</th>
                    <th className="text-left py-2 pr-4 font-semibold text-muted-foreground">Products Bought</th>
                    <th className="text-left py-2 pr-4 font-semibold text-muted-foreground">Email</th>
                    <th className="text-left py-2 pr-4 font-semibold text-muted-foreground">Phone</th>
                    <th className="text-right py-2 font-semibold text-muted-foreground">Actions</th>
                  </tr>
      ...
      <Dialog open={buyerModal} onOpenChange={(o) => { setBuyerModal(o); if (!o) resetBuyerForm(); }}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{editingBuyerId ? "Edit Buyer" : "Add Buyer"}</DialogTitle></DialogHeader>
          <div className="space-y-3 py-2">
            <div className="space-y-1"><Label className="text-xs">Name</Label><Input value={buyerForm.name} onChange={e => setBuyerForm(p => ({ ...p, name: e.target.value }))} data-testid="input-buyer-name" /></div>
            <div className="space-y-1"><Label className="text-xs">Country</Label><Input value={buyerForm.country} onChange={e => setBuyerForm(p => ({ ...p, country: e.target.value }))} data-testid="input-buyer-country" /></div>
            <div className="space-y-1"><Label className="text-xs">Email</Label><Input type="email" value={buyerForm.email} onChange={e => setBuyerForm(p => ({ ...p, email: e.target.value }))} data-testid="input-buyer-email" /></div>
            <div className="space-y-1"><Label className="text-xs">Phone</Label><Input value={buyerForm.phone} onChange={e => setBuyerForm(p => ({ ...p, phone: e.target.value }))} data-testid="input-buyer-phone" /></div>
            <div className="space-y-1"><Label className="text-xs">Address</Label><Input value={buyerForm.address} onChange={e => setBuyerForm(p => ({ ...p, address: e.target.value }))} data-testid="input-buyer-address" /></div>
            <div className="space-y-1.5">
              <Label className="text-xs">Products the Buyer Buys</Label>
              <p className="text-[0.65rem] text-muted-foreground">These products will appear automatically when this buyer is selected on an Export Purchase Order.</p>
              <div className="flex gap-2">
                <Input
